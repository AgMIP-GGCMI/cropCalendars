...
wfdei.path <- "/p/projects/lpjml/input/historical/GGCMI_phase1/WFDEI/"
...
tas_fn1 <- paste0(wfdei.path, "tas_wfdei_1979-2012.clm2")
pr_fn1  <- paste0(wfdei.path, "prtotal_cru_wfdei_1979-2012.clm2")
...
pr_scalar <- 1
  ...
  tas <- read.LPJmL.clim.input(tas_fn1, 1979, SYgp, EYgp, 67420, 43, 365)*tas_scalar
  ...
  pr <- read.LPJmL.clim.input(pr_fn1, 1979, SYgp, EYgp, 67420, 43, 365)*pr_scalar
...
ggplot.map.continuous <- function (dfMap, Value,
                                   xlim = NULL, ylim = NULL,
                                   mainTitle=Value, legendTitle=Value,
                                   ...) {
...
# plot maps  ----
pdf(paste0(climate.dir, "map_climate_", GCM, "_", SCgp, "_", SYgp, "_", EYgp, ".pdf"), width = a4w, height = a4h)
