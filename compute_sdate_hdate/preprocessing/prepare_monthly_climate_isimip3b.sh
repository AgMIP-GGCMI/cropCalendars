...
for gc in "${!gcms[@]}";do
  for sc in "${!scens[@]}";do
    for yy in "${!syears[@]}";do
        ...
        sbatch --qos=standby --ntasks=1 --cpus-per-task=12 -t 01:00:00 -J mclm -A macmit --workdir=${wd} -e out_err/mclm.%j.err -o out_err/mclm.%j.out R --file=preprocessing/prepare_monthly_climate_isimip3b.R --args "${gcms[gc]}" "${scens[sc]}" "${syears[yy]}" "${eyears[yy]}"
