...
ggplot.map.general <- function (dfMap, Value, landFill = "white", #declare Value="colname"
                                xlim = NULL, ylim = NULL,
  ...
  if (!is.na(color_scale[1])) {
    ...
    if (scale_type=="categorical"){
      p <- p + scale_fill_manual(..., values = color_scale, drop=FALSE) # drop=F displays all levels
    } else {
      # p <- p + scale_fill_gradientn(oob=squish, ...)  #oob=squish to constrain scale limits if declared
